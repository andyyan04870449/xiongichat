  ...
  return (
    <div className="fixed inset-0 overflow-hidden" style={{
      ...
      <div className="relative z-10 flex flex-col h-full">
        {/* Header */}
        <header className="p-3 md:p-8 flex-shrink-0">
          <div className="flex items-center space-x-2 md:space-x-6">
            <div className="bg-transparent rounded-xl md:rounded-2xl p-1 md:p-3">
              <img src={logo} alt="雄i聊" className="h-10 md:h-16 w-auto" />
            </div>
            <div className="text-gray-700">
              <h1 className="text-lg md:text-3xl font-bold mb-0.5 md:mb-2" style={{ color: 'var(--theme-text)' }}>
                雄i 聊智能客服機器人
              </h1>
              <p className="text-xs md:text-xl" style={{ color: 'var(--theme-text-secondary)' }}>
                高雄市毒品防制局
              </p>
              <p className="text-xs md:text-lg mt-0 md:mt-1" style={{ color: 'var(--theme-text-secondary)' }}>
                專業關懷 · 陪伴康復
              </p>
            </div>
          </div>
        ...
        {/* Main Content */}
        <div className="flex-1 flex items-center justify-center px-4 md:px-8 overflow-hidden min-h-0">
          <div className="max-w-6xl w-full">
            {/* Mobile and Desktop Layout */}
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
              {/* Welcome content */}
              <div className="flex-1 max-w-lg mx-auto lg:mx-0 lg:max-w-lg space-y-3 md:space-y-12">
                {/* Main Welcome */}
                <div className="text-center space-y-2 md:space-y-8">
                  {/* 手機版熊熊圖示 */}
                  <div className="block md:hidden">
                    <div className="inline-flex items-center justify-center w-32 h-32 bg-white/20 rounded-full mb-2">
                      <img src={bearAvatar} alt="雄i聊熊熊" className="w-24 h-24 object-contain" />
                    </div>
                  </div>
                  <div>
                    <div className="min-h-[80px] md:min-h-[200px] flex items-center justify-center">
                      <p className="text-sm md:text-2xl leading-relaxed text-center whitespace-pre-line" style={{ color: 'var(--theme-text)' }}>
                        {displayedText}
                ...
                <div className="flex justify-center space-x-3 md:space-x-8">
                  <div className="text-center">
                    <div className="w-10 h-10 md:w-16 md:h-16 bg-white/15 rounded-full flex items-center justify-center mb-1 md:mb-3">
                      <Heart className="w-4 h-4 md:w-8 md:h-8" style={{ color: 'var(--theme-primary)' }} />
                    </div>
                    <p className="text-xs md:text-sm" style={{ color: 'var(--theme-text)' }}>用愛陪伴</p>
                  </div>
                  <div className="text-center">
                    <div className="w-10 h-10 md:w-16 md:h-16 bg-white/15 rounded-full flex items-center justify-center mb-1 md:mb-3">
                      <Shield className="w-4 h-4 md:w-8 md:h-8" style={{ color: 'var(--theme-secondary)' }} />
                    </div>
                    <p className="text-xs md:text-sm" style={{ color: 'var(--theme-text)' }}>安全保密</p>
                  </div>
                  <div className="text-center">
                    <div className="w-10 h-10 md:w-16 md:h-16 bg-white/15 rounded-full flex items-center justify-center mb-1 md:mb-3">
                      <Users className="w-4 h-4 md:w-8 md:h-8" style={{ color: 'var(--theme-accent)' }} />
                    </div>
        ...
        <div className="bg-white border-t border-gray-200 p-3 md:p-8 flex-shrink-0">
          <div className="max-w-6xl mx-auto">
            {/* Mobile Layout */}
            <div className="block md:hidden space-y-2">
              <div className="flex items-center justify-center space-x-2 mb-2">
                <Shield className="w-4 h-4" style={{ color: 'var(--theme-primary)' }} />
                <label className="text-gray-700 font-semibold text-sm">通關密碼</label>
              </div>
              <div className="space-y-2">
                ...
                <Button 
                  onClick={handleLogin}
                  className="w-full font-semibold py-2 h-10 text-sm text-white"
                  style={{
        ...
        {/* Footer */}
        <div
          className="text-center py-2 md:py-6 flex-shrink-0"
          style={{ 
            backgroundColor: `${CSS.supports('color', 'var(--theme-accent)') ? 'var(--theme-accent)' : '#d97706'}CC`
          }}
        >
          <div className="flex items-center justify-center space-x-2 mb-1">
            <Heart className="w-3 h-3 md:w-5 md:h-5 text-white/80" />
            <span className="text-white font-medium text-xs md:text-base">雄i 聊 · 用愛陪伴</span>
          </div>
